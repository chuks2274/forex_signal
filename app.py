    ...
    try:
        ...
        for pair in PAIRS:
            try:
                ...
                logger.error(f"[Breakout] Error checking {pair}: {e}", exc_info=True)
        ...
        if breakout_pairs:
            ...
            if send_telegram(alert_msg):
                logger.warning(f"[Breakout] H1 breakout alert sent for pairs: {', '.join(breakout_pairs)}")
        ...
        for currency, breakout_list in group_alert_results.items():
            if breakout_list and isinstance(breakout_list, list) and len(breakout_list) >= 3:
                logger.warning(f"✅ Sent {currency} breakout alert: {', '.join(breakout_list)}")

        # Update last_group_alerts tracker
        ...
        for ev in events:
            ...
            if last_news_alert_times.get(event_key):
